...
struct inode *wrapfs_iget(struct super_block *sb, struct inode *lower_inode)
{
	...
	if (S_ISDIR(lower_inode->i_mode))
		inode->i_fop = &wrapfs_dir_fops;
	    	...
	    	if(WRAPFS_SB(sb)->mmap_option_set == 1) {
		     
				
		    inode->i_fop = &wrapfs_main_mmap_fops;
		}
		else {
	...
	printk("VALUE OF MMAP OPTION IS %d\n",WRAPFS_SB(sb)->mmap_option_set);
	if(WRAPFS_SB(sb)->mmap_option_set == 1) {

	    inode->i_mapping->a_ops = &wrapfs_mmap_aops;
	}   
	else {
